#' Download SYNOP data from Météo-France open data
#'
#' Downloads SYNOP surface observation data from the Météo-France open data
...
#' Get Météo-France API access token
#'
#' Request an OAuth2 access token from the Météo-France API using the
#' client credentials authentication flow.
#'
#' Requires a valid account on the Météo-France API portal \url{https://portail-api.meteofrance.fr} and
...
#' The `client_auth` value must be obtained from your account on the Météo-France API portal:
...
#'   Météo-France API portal (string following "Authorization: Basic").
#'
...
#' Order climate data for a station from the Météo-France API
#'
#' Submit an asynchronous order for climatological data from a
#' Météo-France station. The function returns a order ID that must be used
...
#' Download a climate data file from the Météo-France API
#'
...
#' Download archived meteorological data from the Météo-France "Données Climatologiques" API in one step
#'
...
#'   Météo-France API portal (optional if \code{token} is provided).
#' @param token Character string. OAuth2 access token obtained with
...
#' Get the list of Météo-France stations by department
#'
#' Retrieve the list of available Météo-France stations for a given French
...
#'   \code{mfclim_get_token()}.
#' @param client_auth Character string. Authentication string provided by the
#'   Météo-France API portal. Used to automatically obtain a token if
...
#' Get information about a Météo-France station
#'
#' Retrieve metadata and information for a given Météo-France station
...
#' Check HTTP response status from Météo-France API
#'
...
#' Parse a JSON-like list from Météo-France API into R structures
#'
#' Converts a parsed JSON object (list) returned by the Météo-France API
